...
argument-hint: "[file] -- path to source file to queue for processing"
---
...
The target MUST be a file path. If no target provided, list `inbox/` contents and ask which to queue.

**START NOW.** Queue the source file for processing.
...
**Source is a URL or non-file:** Report error: "/queue requires a file path."
...
Search for content overlap via keyword search in `notes/`:
```bash
...
## Step 5: Determine Note Numbering

Find the highest existing note number across the queue and archive to ensure globally unique note IDs.

```bash
# Check queue for highest note number in file references
QUEUE_MAX=$(grep -oE '[0-9]{3}\.md' ops/queue*.yaml ops/queue/*.yaml 2>/dev/null | \
  grep -oE '[0-9]{3}' | sort -n | tail -1)
QUEUE_MAX=${QUEUE_MAX:-0}

# Check archive for highest note number
...
# Next note starts after the highest
NEXT_NOTE_START=$((QUEUE_MAX > ARCHIVE_MAX ? QUEUE_MAX + 1 : ARCHIVE_MAX + 1))
```

Note numbers are globally unique and never reused across batches. This ensures every task file name (`{source}-{NNN}.md`) is unique vault-wide.
...
next_note_start: {NEXT_NOTE_START}
---
...
## Acceptance Criteria
- Extract insights, implementation ideas, tensions, and testable hypotheses
...
**If no queue file exists:** Create `ops/queue/queue.json` with this first task entry wrapped in `{"tasks": [...]}`.
...
Notes will start at: {NEXT_NOTE_START}
Task files will be: {SOURCE_BASENAME}-{NNN}.md (unique across vault)
Queue: updated with extract task

Next steps:
  /extract {SOURCE_BASENAME}     (extract notes from source)
...
- Handles note numbering automatically (globally unique)
...
- Note task files: `{source-basename}-{NNN}.md`
- Summary: `{source-basename}-summary.md`
- Archive folder: `{date}-{source-basename}/`

Note numbers (NNN) are globally unique across all batches, ensuring every filename is unique vault-wide. This is required because wiki links resolve by filename, not path.
...
When archive runs later, it moves task files into the existing archive folder and generates a summary.
...
**No queue file:** Create `ops/queue/queue.json` with this first entry.
...
- Reuse note numbers from previous batches (globally unique is required)
...
- Compute next_note_start from both queue AND archive (not just one)
