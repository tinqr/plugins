---
name: marrow:check
description: Schema validation for a single note. Checks required fields, description quality, link health, and composability. Non-blocking -- warns but doesn't prevent capture. Triggers on "/check", "/check [note]", "check schema", "validate note".
user-invocable: true
context: fork
model: sonnet
...
1. Check the note's `type` field in frontmatter
2. Look for matching template in `templates/` (e.g. `templates/note.md`, `templates/index.md`)
3. If no template found, use the default schema checks below
...
| `area` | Values matching vault's configured domains | WARN |
...
**FAIL blocks pipeline completion.** A note with any FAIL-level issue should be flagged for fixes.

**WARN does not block.** Warnings are quality signals, not gates.
